    ...
    def update(self, batch_logits, batch_loss, batch, batch_metadata):
        ...
        self.str_list.extend(
            [
                s[i1 : i2 + 1]
                for i1, i2, s in zip(
                    pred_char_start, pred_char_end, batch.selection_str
                )
            ]
        )
...
class SpanPredictionF1andEMScheme(BaseEvaluationScheme):
    ...
    def compute_metrics_from_preds_and_labels(cls, preds, labels):
        em = sum(
            [exact_match_score(s1, s2) for s1, s2 in zip(preds, labels)]
        ) / len(labels)
        f1 = sum(
            [string_f1_score(s1, s2) for s1, s2 in zip(preds, labels)]
        ) / len(labels)
    ...
    def compute_metrics_from_accumulator(
        self,
        task,
        accumulator: ConcatenateStringListAccumulator,
        tokenizer,
        labels: list,
    ) -> Metrics:
...
class MLMPremaskedAccumulator(BaseAccumulator):
    ...
    def update(self, batch_logits, batch_loss, batch, batch_metadata):
        ...
        masked_tokens_selector = (
            batch.masked_lm_labels.cpu().numpy()
...
class MCTACOEvaluationScheme(BaseLogitsEvaluationScheme):
    ...
    @classmethod
    def compute_metrics_from_accumulator(
        self, task, accumulator, tokenizer, labels
    ) -> Metrics:
...
class MultiLabelAccAndF1EvaluationScheme(BaseLogitsEvaluationScheme):
    ...
    def compute_metrics_from_preds_and_labels(cls, preds, labels):
        ...
        minor = {
            ...
            "acc_and_f1_micro": (
                acc + f1_score(y_true=labels, y_pred=preds, average="micro")
            )
            / 2,
        }
...
class MultipleChoiceAccuracyEvaluationScheme(BaseLogitsEvaluationScheme):
    ...
    def get_labels_from_examples(cls, task, examples):
        return get_multiple_choice_label_ids_from_examples(
...
class ReCordEvaluationScheme(BaseEvaluationScheme):
    ...
    def get_preds_from_accumulator(cls, task, accumulator):
        ...
        max_logits = {}
        for logit, entity_str, question_id in zip(
            logits, entity_strs, question_ids
        ):
            if (question_id not in max_logits) or (
                max_logits[question_id]["logit"][1] < logit[1]
            ):
    ...
    def compute_preds_and_metrics(cls, task, accumulator):
        ...
        gold_labels = {}
        for question_id, gold_label_set in zip(
            question_ids, gold_label_list_of_sets
        ):
    ...
    def metric_max_over_ground_truths(
        cls, metric_fn, prediction, ground_truths
    ):
...
class CCGEvaluationScheme(BaseEvaluationScheme):
    ...
    def get_label_ids_from_cache(cls, cache):
        return [
            ...
            # {"tag_ids": datum["data_row"].label_ids, "label_mask": datum["data_row"].label_mask}
            for datum in cache.iter_all()
        ]
    ...
    def compute_metrics_from_preds_and_labels(cls, preds, labels):
        ...
        # label_ids = np.stack([row["tag_ids"] for row in labels])
    ...
    @classmethod
    def compute_metrics_from_flat_preds_and_labels(
        cls, flat_preds, flat_labels
    ):
        return SimpleAccuracyEvaluationScheme.compute_metrics_from_preds_and_labels(
            preds=flat_preds,
            labels=flat_labels,
...
class F1TaggingEvaluationScheme(BaseEvaluationScheme):
    ...
    def compute_metrics_from_preds_and_labels(cls, task, preds, labels):
        ...
        for i in range(len(labels)):
            ...
            relevant_preds_pos = [
                task.ID_TO_LABEL[pos_id] for pos_id in relevant_preds
            ]
            preds_for_eval.append(relevant_preds_pos)

        minor = {
            "precision": seqeval_metrics.precision_score(
                labels_for_eval, preds_for_eval
            ),
...
class TaggingEvaluationScheme(BaseEvaluationScheme):
    def get_accumulator(self):
        return ConcatenateLogitsAccumulator()
    ...
    def get_labels_from_cache_and_examples(cls, task, cache, examples):
        ...
        for datum in cache.iter_all():
            ...
            # import ipdb; ipdb.set_trace()
            labels_list = [
                task.ID_TO_LABEL[label]
                for label in datum["data_row"].label_ids[label_mask]
            ]
            label = {
            ...
            labels.append(label)
            assert len(labels_list) == label_mask.sum()
        return labels
    ...
    def compute_metrics_from_preds_and_labels(cls, task, preds, labels):
        ...
        # import ipdb; ipdb.set_trace()
        labels_for_eval = [label["labels_list"] for label in labels]
        preds_for_eval = []
        assert len(labels) == preds.shape[0]
        for i in range(len(labels)):
            relevant_preds = preds[i][label_mask[i]]
            relevant_preds_label = [
                task.ID_TO_LABEL[label] for label in relevant_preds
            ]
            preds_for_eval.append(relevant_preds_label)
...
class SQuADEvaluationScheme(BaseEvaluationScheme):
    ...
    def get_labels_from_cache(cls, cache):
        return [
            cls.get_label_from_data_row(datum["data_row"])
            for datum in cache.iter_all()
        ]
    ...
    def compute_metrics_from_accumulator(
        ...
        if task.version_2_with_negative:
            ...
            return Metrics(
                major=(results["best_f1"] + results["best_exact"]) / 2,
                minor=results,
            )
        else:
            return Metrics(
                major=(results["f1"] + results["exact"]) / 2, minor=results
            )
...
class MLMPremaskedEvaluationScheme(MLMEvaluationScheme):
    ...
    def get_labels_from_cache_and_examples(cls, task, cache, examples):
        ...
        for datum in cache.iter_all():
            ...
            labels.append(
                masked_lm_labels[
                    masked_lm_labels != mlm_template.NON_MASKED_TOKEN_LABEL_ID
                ]
            )
        return labels
...
class Bucc2018EvaluationScheme(BaseEvaluationScheme):
    ...
    def get_preds_from_accumulator(self, task, accumulator, threshold=0):
        ...
        eng_embeddings = all_embeddings[is_english_arr]
        english_guids = [
            x.split("-", 1)[1] for x in np.array(guids)[is_english_arr]
        ]
        other_guids = [
            x.split("-", 1)[1] for x in np.array(guids)[~is_english_arr]
        ]
        ...
        for score, src_idx, trg_idx in result:
            for src_key, trg_key in itertools.product(
                src_ids_map[src_idx], trg_ids_map[trg_idx]
            ):
...
def get_evaluation_scheme_for_task(task) -> BaseEvaluationScheme:
    ...
    if isinstance(
        ...
        task,
        (
            tasks.CrowdEnventEmotionClassification,
            tasks.CrowdEnventSuddennessClassification,
            tasks.CrowdEnventFamiliarityClassification,
            tasks.CrowdEnventPredictEventClassification,
            tasks.CrowdEnventPleasantnessClassification,
            tasks.CrowdEnventUnpleasantnessClassification,
            tasks.CrowdEnventGoalRelevanceClassification,
            tasks.CrowdEnventChanceResponsbltClassification,
            tasks.CrowdEnventSelfResponsbltClassification,
            tasks.CrowdEnventOtherResponsbltClassification,
            tasks.CrowdEnventPredictConseqClassification,
            tasks.CrowdEnventGoalSupportClassification,
            tasks.CrowdEnventUrgencyClassification,
            tasks.CrowdEnventSelfControlClassification,
            tasks.CrowdEnventOtherControlClassification,
            tasks.CrowdEnventChanceControlClassification,
            tasks.CrowdEnventAcceptConseqClassification,
            tasks.CrowdEnventStandardsClassification,
            tasks.CrowdEnventSocialNormsClassification,
            tasks.CrowdEnventAttentionClassification,
            tasks.CrowdEnventNotConsiderClassification,
            tasks.CrowdEnventEffortClassification,
        ),
    ):
        ...
        return TaggingEvaluationScheme()
        ...
        (
            tasks.StsbTask,
            tasks.CrowdEnventSuddennessRegression,
            tasks.CrowdEnventFamiliarityRegression,
            tasks.CrowdEnventPredictEventRegression,
            tasks.CrowdEnventPleasantnessRegression,
            tasks.CrowdEnventUnpleasantnessRegression,
            tasks.CrowdEnventGoalRelevanceRegression,
            tasks.CrowdEnventChanceResponsbltRegression,
            tasks.CrowdEnventSelfResponsbltRegression,
            tasks.CrowdEnventOtherResponsbltRegression,
            tasks.CrowdEnventPredictConseqRegression,
            tasks.CrowdEnventGoalSupportRegression,
            tasks.CrowdEnventUrgencyRegression,
            tasks.CrowdEnventSelfControlRegression,
            tasks.CrowdEnventOtherControlRegression,
            tasks.CrowdEnventChanceControlRegression,
            tasks.CrowdEnventAcceptConseqRegression,
            tasks.CrowdEnventStandardsRegression,
            tasks.CrowdEnventSocialNormsRegression,
            tasks.CrowdEnventAttentionRegression,
            tasks.CrowdEnventNotConsiderRegression,
            tasks.CrowdEnventEffortRegression,
        ),
    ):
...
def get_multiple_choice_label_ids_from_examples(task, examples):
    return np.array(
        [task.CHOICE_BIMAP.a[example.label] for example in examples]
    )
